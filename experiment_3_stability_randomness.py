...
def get_arguments():
    ...
    parser.add_argument('--dataset_path', dest='dataset_path', type=str,
                        help="The path to the results directory of the dataset for which the correlations shall "
                             "be analysed.")
    parser.add_argument("--jitter_amount", dest='jitter_amount', type=float, default=0.0,
                        help="The jitter amount that shall be fixed. Defaults to 0.0")
    ...
    return args.dataset_path, args.jitter_amount, args.n_jobs


def main():
    dataset_path, jitter_amount, n_jobs = get_arguments()
    random_seed_dirs = []
    ...
    for random_seed_dir in os.listdir(dataset_path):
        if "random_seed" in random_seed_dir:
    ...
    for random_seed_dir in random_seed_dirs:
        for jitter_dir in os.listdir(random_seed_dir):
            if "jitter_amount" in jitter_dir and str(jitter_amount) in jitter_dir:
                random_seed_dirs_correct_jitter.append(os.path.join(random_seed_dir, jitter_dir))
                break  # Only one jitter dir per random_seed_dir
    ...
    for dir1, dir2 in dir_pairs:
        ...
        seed1 = [sub_dir.split("_")[-1] for sub_dir in dir1.split(os.path.sep) if "random_seed" in sub_dir][0]
        seed2 = [sub_dir.split("_")[-1] for sub_dir in dir2.split(os.path.sep) if "random_seed" in sub_dir][0]

        for common_identifier in tqdm(common_file_identifiers):
            layout1_path = os.path.join(dir1, [file for file in os.listdir(dir1)
                                               if common_identifier in file and file.endswith(".npy")][0])
            layout2_path = os.path.join(dir2, [file for file in os.listdir(dir2)
                                               if common_identifier in file and file.endswith(".npy")][0])
            key = (layout1_path, layout2_path)
            processes.append((seed1, seed2, Process(target=get_all_metrics_stability,
    ...
    df_name = "results_experiment_3_" + dataset_name + ".csv"
    ...
    for i, (seed1, seed2, process, key, return_dict) in enumerate(tqdm(processes)):
        if (df is not None and int(seed1) in df["seed_1"].values.tolist() and int(seed2) in df["seed_2"].values.tolist()
                and get_rel_path(list(key), dataset_name) in relativ_df_path_list):
            write_if_appropriate(df_name, i, overall_results)
            continue

        process.start()
        running_jobs.append((process, key, seed1, seed2, return_dict))
...
def write_if_appropriate(df_name, i, overall_results, write_anyway=False):
    if (i + 1) % 10 == 0 or write_anyway:  # Write results every 100 iterations
        df_new = pd.DataFrame(overall_results, columns=["seed_1", "seed_2", "spearman_correlation",
                                                        "pearson_correlation",
